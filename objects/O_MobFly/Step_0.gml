...
if (hp == 0) {
	if (typeof(O_Global.mobsFly) == "array") {
		for (var i = 0; i < array_length(O_Global.mobsFly); i++) {
			if (array_get(O_Global.mobsFly, i) == id) {
				array_delete(O_Global.mobsFly, i, 1);
			}
		}
	}
	instance_destroy();
...
if (hp > 0) {
	...
	var visionCollision = collision_line(x, y, target.x, target.y, O_ColliderFly, true, false);	
	if (visionZone > 0 && !(visionCollision > 0)) {
		if (xTarget32 != floor(target.x/32) || yTarget32 != floor(target.y/32)) {
			...
			SC_GoToBefore(O_Global.flyGird, path, x, y, target.x, target.y);
		}
	}
	else {
		xTarget32 = 0;
		...
		if (!move) {
			...
			if (action < 0.01) {
				...
				if (pointTarget[0] != 0 && pointTarget[1] != 0) {
					move = SC_GoTo(O_Global.flyGird, path, x, y, pointTarget[0], pointTarget[1]);
				}
			}
		}
	}
