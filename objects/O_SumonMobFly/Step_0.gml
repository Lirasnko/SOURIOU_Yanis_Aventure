...
if (hp == 0) {
	if (x != xstart && y != ystart) {
			SC_GoTo(O_Global.sumonFlyGrid, path, x, y, xstart, ystart);
	}
	hp = -1;
}
if (hp == -1 && x == xstart && y == ystart) {
	...
	if (typeof(O_Global.sumonMobsFly) == "array") {
		for(var i = 0; i < array_length(O_Global.sumonMobsFly); i++) {
			if(array_get(O_Global.sumonMobsFly, i) == id) {
				array_delete(O_Global.sumonMobsFly, i, 1);
			}
		}
	}
	instance_destroy();
	...
	if (playerInZone > 0) {
		...
		if (target != noone) {
			...
			if (xTarget32 != floor(target.x/32) || yTarget32 != floor(target.y/32)) {
				...
				SC_GoToBefore(O_Global.sumonFlyGird, path, x, y, target.x, target.y);
			}
		}
		else {
			xTarget32 = 0;
			...
			if (!move) {
				...
				if (action < 0.01) {
					...
					if (pointTarget[0] != 0 && pointTarget[1] != 0) {
						move = SC_GoTo(O_Global.sumonFlyGird, path, x, y, pointTarget[0], pointTarget[1]);
					}
				}
			}
		}
